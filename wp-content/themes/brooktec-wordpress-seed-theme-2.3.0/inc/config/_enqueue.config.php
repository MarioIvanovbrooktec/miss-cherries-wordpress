...
add_action('wp_enqueue_scripts', function () {
    ...
    if (file_exists(get_stylesheet_directory() . '/dist/css/vendor.css')) {
        wp_enqueue_style(
            SEED_THEME_SLUG . '/css/vendor',
            "{$stylesheet_directory_uri}/dist/css/vendor.css",
            array(),
            $theme_version
        );
        $main_style_deps[] = SEED_THEME_SLUG . '/css/vendor';
    }
    wp_enqueue_style(
        SEED_THEME_SLUG . '/css/main',
        "{$stylesheet_directory_uri}/dist/css/main.css",
        $main_style_deps,
        $theme_version
    );
    wp_enqueue_script(
        SEED_THEME_SLUG . '/js/main',
...
add_action('login_enqueue_scripts', function () {
    ...
    if (file_exists(get_stylesheet_directory() . '/dist/js/login.css')) {
        wp_enqueue_style(SEED_THEME_SLUG . '/login', $stylesheet_directory_uri . '/dist/css/login.css', array());
    }
    if (file_exists(get_stylesheet_directory() . '/dist/js/login.min.js')) {
        wp_enqueue_script(
            SEED_THEME_SLUG . '/login-scripts',
            $stylesheet_directory_uri . '/dist/js/login.min.js',
            array('jquery'),
            $theme_version,
            true
        );
        wp_localize_script(SEED_THEME_SLUG . '/login-scripts', 'login_theme_data', array(
            'ajax_url' => admin_url('admin-ajax.php'),
            'site_url' => site_url(),
        ));
    }
